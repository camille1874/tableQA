...
class HardEM(DecoderTrainer[Tuple[torch.Tensor, torch.Tensor]]):
    """
    ...
    def _take_mml(self, loss_tensor):
        return torch.log(torch.sum(torch.exp(loss_tensor - 1e10 * (loss_tensor == 0).float())))

    def _take_min(self, loss_tensor):
        return torch.min(
            loss_tensor + 2 * torch.max(loss_tensor) * (loss_tensor == 0).type(torch.FloatTensor).cuda())
    ...
    def decode(self,
        ...
        if training:
            self.step += 1
            print(self.step)
            if numpy.random.random() < min(self.step / self.tau, 0.8):
                print("Using the MLP as the loss function")
                for instance_states in finished_states.values():
                    scores = [-state.score[0].view(-1) for state in instance_states]
                    new_score = torch.cat(scores)
                    loss += self._take_min(new_score)
                return {'loss': loss / len(finished_states)}
            else:
                print("Using the MML as the loss function")
                for instance_states in finished_states.values():
                    scores = [-state.score[0].view(-1) for state in instance_states]
                    new_score = torch.cat(scores)
                    loss += self._take_mml(new_score)
                return {'loss': loss / len(finished_states)}
        else:
            for instance_states in finished_states.values():
                scores = [-state.score[0].view(-1) for state in instance_states]
