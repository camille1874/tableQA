...
def search(tables_directory: str,
    ...
    for instance_data in data:
        # instance_data["question"]: "how many votes did deodar receive?"
        utterance = instance_data["question"]
        # instance_data["id"]: "nt-202"
        ...
        # context: (table_data_with_column_types, column_types, question_tokens)
        # table_data_with_column_types: List[Dict[str, CellValueType]], 列表中每一个元素代表table每一行。字典中key值为
        # column的name以及对应的type，value为cell的内容，key值按column原始顺序排序，string_column:year:2001
        # column_types: Dict[str, Set[str]] key值为column的名字，value值是一个字典，里边包含该column对应的可能的type,主要有
        # 四类{string, number, num2, date}
        # question_tokens: List[Token]: tokenize过后的question
...
if __name__ == "__main__":
    ...
    parser.add_argument("--table_directory", type=str, default="/home/geshi/nsm_allen/WikiTableQuestions/", help="Location of the 'tagged' directory in the"
                        "WikiTableQuestions dataset")
    parser.add_argument("--data_file", type=str, default="/home/geshi/nsm_allen/WikiTableQuestions/data/training.examples", help="Path to the *.examples file")
    parser.add_argument("--output_path", type=str, default="/home/geshi/code_test/", help="""Path to the output directory if
    ...
    parser.add_argument("--output-separate-files", dest="output_separate_files",
                        ...
                        train a parser.""")
    parser.add_argument("--num-splits", dest="num_splits", type=int, default=20,
                        help="Number of splits to make of the data, to run as many processes (default 20)")
