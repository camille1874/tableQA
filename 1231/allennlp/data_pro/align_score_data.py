...
input_examples_file = "/home/xuzh/mnt/allennlp/nsm_allen/WikiTableQuestions/data/random-split-1-train-with-lgfs.examples"
input_examples_file_aligned = "/home/xuzh/mnt/allennlp/nsm_allen/WikiTableQuestions/data/random-split-1-train-aligned.examples"
input_examples_file_noaligned = "/home/xuzh/mnt/allennlp/nsm_allen/WikiTableQuestions/data/random-split-1-train-noaligned.examples"
table_path = "/home/xuzh/mnt/allennlp/nsm_allen/WikiTableQuestions/"

lines = open(input_examples_file)
new_file_aligned = open(input_examples_file_aligned, 'w')
new_file_noaligned = open(input_examples_file_noaligned, 'w')
data_examples = {parse_example_line(example_line)['id']: parse_example_line(example_line) for example_line in lines}
lines = lines.readlines()

aligned = []
noaligned =[]
for lgs_path in os.listdir(lgs_dir):
    if not lgs_path.split('.')[0] in data_examples:
        continue
    ...
    #            fmodel.write(lgf)
        if score == 2.0:
            aligned.append(data_examples[lgs_path.split('.')[0]]['raw'])
            break
    else:
        noaligned.append(data_examples[lgs_path.split('.')[0]]['raw'])
new_file_aligned.write("".join(aligned))
new_file_noaligned.write("".join(noaligned))
