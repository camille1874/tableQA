...
from allennlp.state_machines.data_helpers import clean_str
import logging
...
class Word2Vec():
    def __init__(self):
        self.model = gensim.models.KeyedVectors.load_word2vec_format('/home/xuzh/new1/allennlp/allennlp/state_machines/GoogleNews-vectors-negative300.bin',
...
class MData(Data):
    def open_file(self, data_file):
        data = codecs.open(data_file, encoding="utf-8").readlines()
        self.features = [[float(x.strip().split("\t")[-2])] for x in data]
        self.labels = [int(x.strip().split("\t")[-1]) for x in data]
    ...
    def open_file_final(self, x_file, y_file):
        ...
        self.features = [[float(x.strip().split("\t")[-1])] for x in xs]
        xs = [clean_str("\t".join(x.split("\t")[:-1])).split(" ")[:self.max_len] for x in xs]
        for i in range(5):
            logger.info(xs[i])
        new_y = [int(x.strip()) for x in ys]
        self.s = np.array(xs)
    ...
    def get_data(self, question, table, lgf, predict_score):
        xs = question + "\t" + table + "\t" + lgf
        xs = [clean_str(xs).split(" ")[:self.max_len]]
        self.s = np.array(xs)
        self.features = [[predict_score]]
