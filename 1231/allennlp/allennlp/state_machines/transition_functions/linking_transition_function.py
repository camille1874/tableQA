...
from allennlp.state_machines.util import get_weights
from allennlp.state_machines.util import Word2Vec
...
class LinkingTransitionFunction(BasicTransitionFunction):
    ...
    def __init__(self,
        ...
        weight = torch.FloatTensor(action_embedding_dim, encoder_output_dim)
        self.align_weight = torch.nn.Parameter(weight, requires_grad=True)
        torch.nn.init.xavier_uniform_(self.align_weight) 
    ...
    def _compute_action_probabilities(self,
        ...
        for group_index in range(group_size):
            ...
            if 'global' in instance_actions:
                ...
                fixed_action_embeddings = instance_actions['fixed']
                #embedded_action_weights, _ = torch.max(fixed_action_embeddings.mm(encoder_outputs[group_index].transpose(-1, 0)), 1)
             
                logger.info("#" * 50)
                logger.info(question) 
                #logger.info("embedded_ques")
                #logger.info(embedded_ques)
                #logger.info("fixed_action_embeddings")
                #logger.info(fixed_action_embeddings)
                embedded_action_weights, _ = torch.max(fixed_action_embeddings.mm(self.align_weight).mm(embedded_ques.squeeze(0).transpose(-1, 0)), 1)
                ...
                action_strings = [action_mapping[(state.batch_indices[group_index], action_index)] for action_index in action_ids] 
                for i in range(len(action_strings)):
                    logger.info(action_strings[i])
                    logger.info(embedded_action_weights[i])

                #logger.info("embedded_action_weights")
                #logger.info(embedded_action_weights.size())
                #logger.info(embedded_action_weights)
                #logger.info("embedded_action_logits")
                #logger.info(embedded_action_logits.size())
                #logger.info(embedded_action_logits)

                #embedded_action_weights = get_weights(state.batch_indices[group_index], action_ids, action_mapping, question, self.w2v_model)
                embedded_action_logits = embedded_action_logits.mul(embedded_action_weights)
                
                #logger.info("new_embedded_action_logits")
                #logger.info(embedded_action_logits.size())
                #logger.info(embedded_action_logits)
            ...
            if 'linked' in instance_actions:
                ...
                if self._mixture_feedforward is not None:
                    ...
                    if embedded_action_logits is not None:
                        ...
                        #logger.info("entity_action_probs")
                        #logger.info(entity_action_probs)
                        #logger.info("embedded_action_probs")
                        #logger.info(embedded_action_probs)
                    ...
                    if embedded_action_logits is not None:
                        #logger.info("linked_action_logits")
                        #logger.info(linked_action_logits)
                        #logger.info("embedded_action_logits")
                        #logger.info(embedded_action_logits)
                        linked_action_logits = linked_action_logits * 0.1
