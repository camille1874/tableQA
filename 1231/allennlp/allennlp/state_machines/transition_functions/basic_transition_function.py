...
class BasicTransitionFunction(TransitionFunction[GrammarBasedState]):
    ...
    def take_step(self,
                  ...
                  allowed_actions: List[Dict[int, float]] = None,
                  weights: List[float] = None) -> List[GrammarBasedState]:
        ...
        batch_results = self._compute_action_probabilities(state,
                                                           ...
                                                           updated_state['encoder_outputs'],
                                                           updated_state['encoder_output_mask'])
    ...
    def _construct_next_states(self,
                               ...
                               max_actions: int,
                               allowed_actions: List[Dict[int, float]],
                               weights: List[float]):
                ...
                for group_index, log_probs, _, action_embeddings, actions in results:
                    for log_prob, action_embedding, action in zip(log_probs, action_embeddings, actions):
                        if action in allowed_actions[group_index]:
                            ...
                            #if allowed_actions[group_index][action] == 2.0:
                            #    new_states.append(make_state(group_index, action, log_prob, action_embedding))
                            #else:
                            #new_states.append(make_state(group_index, action, log_prob / (1 + float(allowed_actions[group_index][action])), action_embedding))
            else:
    ...
    def _take_first_step(self,
                         state: GrammarBasedState,
                         allowed_actions: List[Dict[int, float]] = None) -> List[GrammarBasedState]:
