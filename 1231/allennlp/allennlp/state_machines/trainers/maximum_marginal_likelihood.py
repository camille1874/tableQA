...
class MaximumMarginalLikelihood(DecoderTrainer[Tuple[torch.Tensor, torch.Tensor]]):
    ...
    def decode(self,
	       ...
	       supervision: Tuple[torch.Tensor, torch.Tensor],
               weights: List[float],
               embedded_ques) -> Dict[str, torch.Tensor]:
        targets, target_mask = supervision
        beam_search = ConstrainedBeamSearch(self._beam_size, targets, target_mask, weights)
        finished_states: Dict[int, List[State]] = beam_search.search(initial_state, transition_function, question, kg, actions, world, embedded_ques)
