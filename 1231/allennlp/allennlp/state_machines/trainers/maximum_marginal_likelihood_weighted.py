...
class MaximumMarginalLikelihood(DecoderTrainer[Tuple[torch.Tensor, torch.Tensor]]):
    ...
    def decode(self,
        ...
        beam_search = ConstrainedBeamSearch(self._beam_size, targets, target_mask, weights)
        ...
        for instance_states in finished_states.values():
            ...
            #weight = 1
            #try:
            #    decoded = instance_states[-1].action_history[0]
            #    references = targets.tolist()[0]
            #    while len(decoded) < len(references[0]):
            #        decoded += [-1]
            #
            #    weight = float(weights[0][references.index(decoded)])
            #    logger.info("weight")
            #    logger.info(weight)
            #except Exception as e:
            #    logger.error(e)
            #loss += (0.001 + weight / 2) * (-util.logsumexp(torch.cat(scores)))
            ##loss += -util.logsumexp(torch.cat(scores))
            decoded = [state.action_history[0] for state in instance_states]
            references = targets.tolist()[0]
            for i in range(len(decoded)):
                while len(decoded[i]) < len(references[0]):
                    decoded[i] += [-1]
            dec_weights = [float(weights[0][references.index(dec)]) for dec in decoded]
            #logger.info("decoded")
            #logger.info(decoded)
            #logger.info("weights")
            #logger.info(dec_weights)
            scores = [a * b for a, b in zip(scores, dec_weights)]
            loss += -util.logsumexp(torch.cat(scores))
