...
def align_column(lgfs_column, question, kg):
    ...
    for item in lgfs_column:
        if "column" in item and item.split(':')[1] not in question:
            column_num += 1
            column_flag = False
        ...
        elif "column" in item and (item.split(':')[1] in question or item.split(':')[1] in instance['question'].replace(" ", "_")):
            column_num += 1
            column_align_num += 1
...
def align(logical_form, question, table):
    ...
    #question = ' '.join(get_lemmatized(question))
    ...
    column_score = align_column(column, question, table)
