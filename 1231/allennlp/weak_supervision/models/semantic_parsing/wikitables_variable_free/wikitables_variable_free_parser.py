...
class WikiTablesVariableFreeParser(Model):
    """
    A ``WikiTablesVariableFreeParser`` is a :class:`Model` which takes as input a table and a question,
    and produces a logical form that answers the question when executed over the table.  The
    logical form is generated by a `type-constrained`, `transition-based` parser. The difference
    between this class and `allennlp.models.semantic_parsing.wikitables.WikiTablesSemanticParser` is
    the language into which the questions are parsed. This one uses a simpler variable free
    language, while the other one uses LambdaDCS.
    Parameters
    ----------
    vocab : ``Vocabulary``
    ...
    def __init__(self,
        ...
        self.fixed_question_embedder = fixed_question_embedder
        ...
        self.fixed_action_embedder = Embedding(num_embeddings=num_actions, embedding_dim=action_embedding_dim, trainable=False)
    ...
    def _get_initial_rnn_and_grammar_state(self,
        ...
        fixed_embedded_question = self.fixed_question_embedder(question)
        ...
        return initial_rnn_state, initial_grammar_state, fixed_embedded_question

    @staticmethod
    def _get_neighbor_indices(worlds: List[WikiTablesVariableFreeWorld],
                              num_entities: int,
        ...
        Parameters
        ----------
        worlds : ``List[WikiTablesVariableFreeWorld]``
        num_entities : ``int``
        tensor : ``torch.Tensor``
    ...
    @staticmethod
    def _get_type_vector(worlds: List[WikiTablesVariableFreeWorld],
                         num_entities: int,
    ...
    def _create_grammar_state(self,
        ...
        for key, action_strings in valid_actions.items():
            ...
            if global_actions:
                ...
                #logger.info("action_embedder")
                #for parameters in self._action_embedder.parameters():
                #    logger.info(parameters)
                
                global_fixed_embeddings = self.fixed_action_embedder(global_action_tensor)
                ...
                translated_valid_actions[key]['fixed'] = global_fixed_embeddings
