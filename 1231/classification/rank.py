...
tf.flags.DEFINE_string("final_x", "./test_x.txt", "Final x")
tf.flags.DEFINE_string("final_y", "./test_y.txt", "Final y")
...
if y_test is not None:
    ...
    neg_sample = []
    pos_sample = []
    new_pos_file = open(os.path.join(FLAGS.checkpoint_dir, "..", 'SQ.filtered_pos_sample.txt'), mode='w')
    new_neg_file = open(os.path.join(FLAGS.checkpoint_dir, "..", 'SQ.filtered_neg_sample.txt'), mode='w')
    
    for i in range(1, len(y_test)):
        if y_test[i] == 1:
            labels.append(tmp_y)
            preds.append(tmp_s)
            pre_ans = tmp_s.index((max(tmp_s)))
            #if pre_ans == 0:
            if (pre_ans == 0 and (min(tmp_s) != tmp_s[pre_ans])) or (len(tmp_s) == 1):                              correct += 1
            else:
                pos_sample.append(x_raw[i - len(tmp_y) + tmp_y.index(1)])
                neg_sample.append(x_raw[i - len(tmp_y) + pre_ans])
            tmp_y = []
            tmp_s = []
